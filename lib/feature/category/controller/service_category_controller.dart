...
class ServiceCategoryController extends GetxController implements GetxService{
    ...
    scrollController.addListener(() {
      if(scrollController.position.maxScrollExtent == scrollController.position.pixels) {
        if(_offset < _pageSize ) {
          if(serviceCategoryList.isNotEmpty) getSubCategoryList(offset: _offset+1,isFromPagination: true);
        }
      }
    });
  }
  ...
  Future<void> getCategoryList() async {
    ...
    if(response.statusCode == 200){
      ...
      if(serviceCategoryList.isNotEmpty)getSubCategoryList(offset: 1, isFromPagination: false);
    }
    else {
