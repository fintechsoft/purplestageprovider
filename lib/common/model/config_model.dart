...
class ConfigContent {
  ...
  ConfigContent.fromJson(Map<String, dynamic> json) {
    ...
    _forgetPasswordVerificationMethod = json['forget_password_verification_method'] != null
        ? ForgetPasswordVerificationMethod.fromJson(json['forget_password_verification_method'])
        : null;
...
class ForgetPasswordVerificationMethod {
  ...
  // Factory method to handle both string and structured JSON input
  static ForgetPasswordVerificationMethod fromJson(dynamic json) {
    final method = ForgetPasswordVerificationMethod();

    // Check if the input is a string
    if (json is String) {
      if (json == 'email') {
        method.email = 1; // Set email as enabled (1 for true)
      } else if (json == 'phone') {
        method.phone = 1; // Set phone as enabled (1 for true)
      }
    } else if (json is Map<String, dynamic>) {
      // Handle structured input in case it changes later
      method.phone = int.tryParse(json['phone'].toString());
      method.email = int.tryParse(json['email'].toString());
    }
